...
gulp.task("setup:analyzer").description = "Installs dependencies to analyze the JS bundle";
gulp.task("setup:aws", shell.task([
  "pip install awscli",
  "aws init",
]));
gulp.task("setup:aws").description = "Installs & initilizes awscli for deploying visuals";
gulp.task("setup:lint", shell.task(
  "npm install --global eslint stylelint"
));
gulp.task("setup:lint").description = "Installs linters to catch style & semantic errors";
...
gulp.task("analyze").description = "Generates a treemap of the JS. Useful for optimizing bundle size.";
...
gulp.task("preview").description = "Open a browser tab to the visual and copy the URL to your clipboard";
...
gulp.task("publish").description = "A series of commands which publishes a visual to AWS S3";
